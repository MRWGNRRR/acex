...
use acex_d::can::IsoTpAddressingMode;
use acex_d::core::Vec;
use acex_d::doip::{
...
use acex_d::server::{handler::ServerHandler, security_provider::SecurityProvider};
use acex_d::sim::{
...
impl ActivationAuthProvider for TestActivationAuthProvider {
    fn authenticate(
        ...
        _source_address: u16,
        _oem_data: &[u8],
    ) -> Result<(), acex_d::doip::session::ActivationDenialReason> {
        Ok(())
    }
}
    ...
    >
{
    ...
    pub fn disconnect(&mut self, tester_addr: NodeAddress, gateway_addr: NodeAddress) {
        ...
        self.tester.on_tcp_event(
            &acex_d::sim::tcp_bus::TcpEvent::ConnectionReset {
                from: tester_addr,
                to: gateway_addr,
            },
            now,
...
///
/// ```no_run,ignore
/// # use acex_tests::fixtures::doip::EcuNodeConfig;
/// # use acex_tests::fixtures::doip::GatewayNodeConfig;
/// # use acex_tests::fixtures::doip::DoipDstScenarioBuilder;
