...
use acex_can::isotp::address::IsoTpAddressingMode;
use acex_can::isotp::reassembler::{ReassembleResult, Reassembler, ReassemblerConfig};
use acex_can::isotp::segmenter::{SegmentResult, Segmenter, SegmenterConfig};
...
fn rt_sequence_error() {
    ...
    assert!(matches!(
        err,
        acex_can::IsoTpError::SequenceError {
            expected: 1,
            actual: 2
        }
    ));
...
fn rt_unexpected_consecutive_frame() {
    ...
    assert!(matches!(
        err,
        acex_can::IsoTpError::UnexpectedConsecutiveFrame
    ));
}
...
fn rt_empty_single_frame() {
    ...
    assert!(matches!(err, acex_can::IsoTpError::EmptySingleFrame));
}
