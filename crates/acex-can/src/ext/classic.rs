...
use acex_core::AddressMode;
use acex_proto::can::frame::classic::{CanFrame, CanFrameMut};
...
pub trait CanFrameExt {
    ...
    fn can_id(&self) -> Option<Result<acex_proto::can::id::CanId, CanError>> {
        let word = self.id_word()?;
        if word & EFF_FLAG != 0 {
            let raw = word & EXT_ID_MASK;
            Some(
                acex_proto::can::id::ExtendedCanId::new(raw)
                    .map(acex_proto::can::id::CanId::Extended)
                    .map_err(CanError::from),
            )
        } else {
            let raw = (word & STD_ID_MASK) as u16;
            Some(
                acex_proto::can::id::StandardCanId::new(raw)
                    .map(acex_proto::can::id::CanId::Standard)
                    .map_err(CanError::from),
            )
        }
    }
...
impl<'a> CanFrameExt for CanFrame<'a> {
    fn as_bytes(&self) -> &[u8] {
        acex_proto::common::RawFrame::as_bytes(self)
    }
}
...
pub trait CanFrameMutExt: CanFrameExt {
    ...
    fn set_standard_id(&mut self, id: &acex_proto::can::id::StandardCanId) -> Result<(), CanError> {
        let word = id.value() as u32 & STD_ID_MASK;
        self.set_id_word(word)
    }

    fn set_extended_id(&mut self, id: &acex_proto::can::id::ExtendedCanId) -> Result<(), CanError> {
...
impl<'a> CanFrameMutExt for CanFrameMut<'a> {
    fn as_bytes_mut(&mut self) -> &mut [u8] {
        acex_proto::common::RawFrameMut::as_bytes_mut(self)
    }
}
