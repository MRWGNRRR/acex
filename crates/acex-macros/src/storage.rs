...
pub fn derive(
        ...
        if attrs.bytes {
            if let Some(length_expr) = &attrs.length {
                owned_decode_stmts.push(quote! {
                    let #ident: ::alloc::vec::Vec<u8> = acex_core::codec::take_n(
                        buf,
                        (#length_expr) as usize,
                    ).map_err(|e| <#error as From<acex_core::DiagError>>::from(e))?.to_vec();
                });
            ...
            owned_encode_stmts.push(quote! {
                acex_core::codec::FrameWrite::encode(&self.#ident, buf)
            ...
            owned_decode_stmts.push(quote! {
                let #ident: ::alloc::vec::Vec<#inner_ty> = {
                    let __slice = *buf;
                    *buf = &buf[buf.len()..];
                    acex_core::codec::FrameIter::<#inner_ty>::new(__slice)
            ...
            owned_encode_stmts.push(quote! {
                for __item in &self.#ident {
                    acex_core::codec::FrameWrite::encode(__item, buf)
                        .map_err(|e| <#error as From<_>>::from(e))?;
                }
            });
            ...
            owned_field_tys.push(quote! { pub #ident: #ty });
            owned_decode_stmts.push(quote! {
                let #ident = <#ty as acex_core::codec::FrameRead>::decode(buf)
                    .map_err(|e| <#error as From<_>>::from(e))?;
            });
            owned_encode_stmts.push(quote! {
                <#ty as acex_core::codec::FrameWrite>::encode(&self.#ident, buf)
                    .map_err(|e| <#error as From<_>>::from(e))?;
            });
            to_owned_stmts.push(quote! { #ident: self.#ident.clone() });
        }
    ...
    quote! {
        ...
        // FrameRead for Owned
        #[cfg(feature = "alloc")]
        impl<'a> acex_core::codec::FrameRead<'a> for #owned_name {
            type Error = #error;
        ...
        #[cfg(feature = "alloc")]
        impl acex_core::codec::FrameWrite for #owned_name {
            type Error = #error;
            fn encode<__W: acex_core::codec::Writer>(
                &self,
                buf: &mut __W,
            ) -> Result<(), Self::Error> {
                #(#owned_encode_stmts)*
        ...
        #[cfg(feature = "alloc")]
        impl #ig acex_core::codec::IntoOwned for #name #tg #wc {
