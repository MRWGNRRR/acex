...
fn derive_struct(
    ...
    quote! {
        impl #ig acex_core::codec::FrameWrite for #name #tg #wc {
            type Error = #error;

            fn encode<__W: acex_core::codec::Writer>(&self, buf: &mut __W) -> Result<(), Self::Error> {
                #(#stmts)*
                Ok(())
            }
        }
        ...
        match &variant.fields {
            ...
            Fields::Unnamed(f) if f.unnamed.len() == 1 => {
                ...
                if let Some(id_expr) = &attrs.id {
                    arms.push(quote! {
                        #name::#vname(inner) => {
                            <#disc_ty as acex_core::codec::FrameWrite>::encode(&(#id_expr as #disc_ty), buf)
                                .map_err(|e| <#error as From<acex_core::DiagError>>::from(e))?;
                            <#inner_ty as acex_core::codec::FrameWrite>::encode(inner, buf)
                                .map_err(|e| <#error as From<_>>::from(e))?;
                        }
                    });
                    ...
                    arms.push(quote! {
                        #name::#vname(raw) => {
                            <#disc_ty as acex_core::codec::FrameWrite>::encode(raw, buf)
                                .map_err(|e| <#error as From<acex_core::DiagError>>::from(e))?;
                        }
                    });
                }
