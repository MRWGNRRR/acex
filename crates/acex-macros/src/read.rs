...
fn derive_struct(
    ...
    for field in named {
        ...
        let stmt = if attrs.skip {
            ...
            quote! {
                let #ident: #ty = acex_core::codec::take_n(
                    buf,
                    (#length_expr) as usize,
                ).map_err(|e| <#error as From<acex_core::DiagError>>::from(e))?;
            }
    ...
    quote! {
        impl #ig acex_core::codec::FrameRead<#decode_lt> for #name #tg #wc {
            type Error = #error;
...
fn derive_enum(
                ...
                if let Some(id_expr) = &attrs.id {
                    quote! {
                        __disc if __disc == (#id_expr) as #disc_ty => {
                            Ok(#name::#vname(
                                <#inner_ty as acex_core::codec::FrameRead>::decode(buf)
                                    .map_err(|e| <#error as From<_>>::from(e))?
                            ))
                        }
                    }
                    ...
                    if attrs.decode_inner {
                        quote! {
                            __disc if matches!(__disc, #pat) => {
                                Ok(#name::#vname(
                                    <#inner_ty as acex_core::codec::FrameRead>::decode(&mut { __saved }).map_err(|e| <#error as From<_>>::from(e))?
                                ))
                            }
                        }
                    } else {
    ...
    arms.push(quote! {
        _ => Err(<#error as From<acex_core::DiagError>>::from(
            acex_core::DiagError::InvalidFrame(
    ...
    quote! {
        impl #ig acex_core::codec::FrameRead<#decode_lt> for #name #tg #wc {
            ...
            fn decode(buf: &mut &#decode_lt [u8]) -> Result<Self, Self::Error> {
                let __saved = *buf;
                let __disc = <#disc_ty as acex_core::codec::FrameRead>::decode(buf)
                    .map_err(|e| <#error as From<acex_core::DiagError>>::from(e))?;
                match __disc {
                    #(#arms)*
                }
