...
use acex_core::{DiagError, FrameIter};
...
impl<'a> acex_core::codec::FrameRead<'a> for ScalingByte<'a> {
    type Error = UdsError;
        ...
        *buf = &buf[1..];
        ...
        let extension = match &high_nibble {
            ScalingByteHighNibble::BitMappedReportedWithoutMask => {
                let mut ext_buf =
                    acex_core::codec::take_n(buf, length).map_err(|e| UdsError::from(e))?;
...
impl acex_core::codec::FrameWrite for ScalingByte<'_> {
    type Error = UdsError;
    fn encode<W: acex_core::codec::Writer>(&self, buf: &mut W) -> Result<(), Self::Error> {
...
impl<'a> acex_core::codec::FrameRead<'a> for BitMappedReportedWithoutMask<'a> {
    type Error = UdsError;
    fn decode(buf: &mut &'a [u8]) -> Result<Self, Self::Error> {
        let mask_byte = *buf
            .first()
...
impl acex_core::codec::FrameWrite for BitMappedReportedWithoutMask<'_> {
    type Error = UdsError;
    fn encode<W: acex_core::codec::Writer>(&self, buf: &mut W) -> Result<(), Self::Error> {
...
impl<'a> acex_core::codec::FrameRead<'a> for StateAndConnectionType {
    type Error = UdsError;
    fn decode(buf: &mut &'a [u8]) -> Result<Self, Self::Error> {
        let byte = *buf
            .first()
...
impl acex_core::codec::FrameWrite for StateAndConnectionType {
    type Error = UdsError;
    fn encode<W: acex_core::codec::Writer>(&self, buf: &mut W) -> Result<(), Self::Error> {
