use crate::UdsError;
use acex_core::{take_n, FrameRead};
use acex_macros::FrameWrite;
...
impl<'a> FrameRead<'a> for RequestDownloadRequest<'a> {
    type Error = UdsError;
...
impl<'a> FrameRead<'a> for RequestDownloadResponse<'a> {
    type Error = UdsError;

    fn decode(buf: &mut &'a [u8]) -> Result<Self, Self::Error> {
        let length_format_identifier = u8::decode(buf)?;

        // High nibble specifies the byte length of max_number_of_block_length.
        // Low nibble is reserved and ignored on decode.
