...
impl<'a> acex_core::codec::FrameRead<'a> for DtcGroup {
    type Error = UdsError;

    fn decode(buf: &mut &'a [u8]) -> Result<Self, Self::Error> {
        let bytes = acex_core::codec::take_n(buf, 3).map_err(|e| UdsError::from(e))?;
...
impl acex_core::codec::FrameWrite for DtcGroup {
    type Error = UdsError;
