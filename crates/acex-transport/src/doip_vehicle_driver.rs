...
impl DoipVehicleDriver {
    ...
    pub fn connect(
        ...
        let now = acex_d::sim::clock::Instant::from_micros(
            SystemTime::now()
    ...
    fn flush_outbox(&mut self) {
        let mut outbox: acex_d::core::Vec<
            (NodeAddress, acex_d::core::Vec<u8, TCP_MAX_FRAME>),
            TCP_MAX_OUTBOX,
        > = acex_d::core::Vec::new();
    ...
    fn wall_clock_now(&self) -> acex_d::sim::clock::Instant {
        acex_d::sim::clock::Instant::from_micros(
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap_or_default()
                .as_micros() as u64,
